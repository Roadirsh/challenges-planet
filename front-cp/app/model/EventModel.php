...
class EventModel extends CoreModel{
    ...
    private function setTopEvent($AllEvent) {
        if(is_array($AllEvent)){
	...
	public function insertNewEvent($post)
	{
		...
		{	
            ...
            */
	        ...
	        $insert->bindParam(':name', $post['name'], PDO::PARAM_STR);
	        $insert->bindParam(':descr', $post['descr'], PDO::PARAM_STR);
            $insert->bindParam(':begin', $post['from'], PDO::PARAM_INT);
            $insert->bindParam(':end', $post['end'], PDO::PARAM_INT);
        ...
        while($i < count($str)){
            ...
            */
            $select = $this->connexion->prepare("SELECT COUNT(*) as event_nb_team
                                            ...
                                            WHERE event_event_id = " . $str[$i]['event_id'] . "
                                            LIMIT 7");
            ...
            */
            $array[$i]= array_merge($str[$i], $CountGroupEvent['0']);
    ...
    // public function getSeeEvent() {
    //     return $this->seeevents;
    // }
    // private function setSeeEvent($AllEvent) {
    //     if(is_array($AllEvent)){
    //         $this->seeevents = $AllEvent;
    //     }
    // }
        ...
        foreach ($group as $key => $e) {
            ...
            if(!empty($AllGroups[0]['group_id'])){
                ...
                $select3 = $this->connexion->prepare("SELECT sum(donate_amount) AS needed
                                                    FROM cp_donate 
                                                    WHERE group_group_id = " . $AllGroups[0]['group_id']);
    ...
    public function SeeOneEvent($eID){
        ...
        try {
            ...
            foreach ($AllGroupsID as $key => $e) {
                ...
                $select2 = $this->connexion->prepare("SELECT *
                                                    FROM " . PREFIX . "group 
                                                    WHERE group_id = " . $e['group_group_id'] . "
                                                    AND group_valid = 1");
