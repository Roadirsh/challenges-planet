...
CREATE TABLE IF NOT EXISTS `giraudsa`.`cp_bank_details` (
  ...
  `cp_user_user_id` INT NOT NULL,
  PRIMARY KEY (`bank_id`, `cp_user_user_id`),
  INDEX `fk_cp_bank_details_cp_user1_idx` (`cp_user_user_id` ASC),
  CONSTRAINT `fk_cp_bank_details_cp_user1`
    FOREIGN KEY (`cp_user_user_id`)
    REFERENCES `giraudsa`.`cp_user` (`user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
DELIMITER $$
USE `giraudsa`$$
CREATE TRIGGER `cp_event_BDEL` BEFORE DELETE ON `cp_event` FOR EACH ROW
BEGIN
INSERT INTO cp_event_trash
   (
     trash_date,
     trash_event_id,
     trash_event_date,
